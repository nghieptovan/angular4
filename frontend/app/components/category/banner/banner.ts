...
export class LtBannerComponent {
    campaign: any;
    selectedProduct: any = {
        'cartItem': {}
    };
    ...
    constructor(
        ...
        this.store.select(fromRoot.commonGetUrlInfo).subscribe((urlInfo) => {
            this.innerHtml = null;
            if (urlInfo.type === 'category') {
                if (urlInfo.category.top_content) {
                    this.innerHtml = this.domSanitizer.bypassSecurityTrustHtml(urlInfo.category.top_content);
                    //const campaignDiv = $('.lt-campaign-products');
                    setTimeout(() => {
                        const campaignDiv = $('.lt-campaign-products');

                        this.appendHtmlProductList(campaignDiv);
                        this.globalService.runScript('top-cate-banner');
